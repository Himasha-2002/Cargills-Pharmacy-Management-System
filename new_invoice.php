      ...
      <div class="container">
        ...
        <div class="row">
          ...
          <div class="row col col-md-12">
            ...
            <div class="col col-md-2 form-group">
              <label class="font-weight-bold" for="customer_id">Customer ID :</label>
              <input id="customer_id" type="text" class="form-control" name="customer_id" placeholder="Customer ID" disabled>
            </div>
            <script>
              document.getElementById('customers_name').addEventListener('blur', function() {
              var customerName = this.value;
              if (customerName) {
                // Assuming you have a function to get customer details by name
                getCustomerDetailsByName(customerName, function(customer) {
                document.getElementById('customer_id').value = customer.id;
                document.getElementById('invoice_number').value = generateInvoiceNumber(customer.id);
                });
              }
              });

              function getCustomerDetailsByName(name, callback) {
              // Simulate an AJAX request to get customer details
              var customer = { id: 'CUST123' }; // Replace with actual AJAX call
              callback(customer);
              }

              function generateInvoiceNumber(customerId) {
              // Generate invoice number based on customer ID
              return 'INV-' + customerId + '-' + new Date().getTime();
              }
            </script>
            <div class="col col-md-2 form-group">
          ...
          <div class="row col col-md-12">
            ...
            <div id="print_button" class="col col-md-2 form-group float-right" style="display: none;">
              <label class="font-weight-bold" for=""></label>
                <button class="btn btn-warning form-control font-weight-bold" onclick="window.location.href='manage_invoice.php';">Show invoices</button>
            </div>
